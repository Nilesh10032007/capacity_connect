...
import { MOCK_SKILL_GAPS, MOCK_HEATMAP_DATA } from '../../../services/mockData';
import { Badge } from '../../common/Badge';
import { BrainCircuit, AlertTriangle, Target, Download, TrendingUp, Users } from 'lucide-react';
import { useApp } from '../../../context/AppContext';

export const CompetencyAnalyticsView: React.FC = () => {
  const { showToast } = useApp();

  const departments = ['Forecasting', 'Observation', 'Climate', 'IT'];
  ...
  return (
    <div className="space-y-6">
      ...
      {/* DEPARTMENT COMPETENCY SUMMARY CARDS (Replaces Heatmap) */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {departments.map((dept) => {
          const deptData = MOCK_HEATMAP_DATA.filter((h) => h.department === dept);
